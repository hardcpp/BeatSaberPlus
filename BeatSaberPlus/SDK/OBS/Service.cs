...
namespace BeatSaberPlus.SDK.OBS
{
    ...
    public partial class Service
    {
        ...
        public static void ApplyConf()
        {
            if (m_ReferenceCount > 0)
            {
                if (BSPConfig.Instance.OBS.Enabled)
                {
                    if (m_Client.IsConnected)
                        m_Client.Disconnect();

                    m_Client.Connect("ws://" + BSPConfig.Instance.OBS.Server);
                }
                else if (!BSPConfig.Instance.OBS.Enabled && Status != EStatus.Disconnected && Status != EStatus.AuthRejected)
                    m_Client.Disconnect();
            }
        }
        ...
        private static void Create()
        {
            ...
            m_Client.ReconnectDelay = 10 * 1000;
        ...
        {
            Logger.Instance.Info("[SDK.OBS][Service.WebSocket_OnOpen]");
        ...
        {
            Logger.Instance.Info("[SDK.OBS][Service.WebSocket_OnClose]");
        ...
        {
#if DEBUG
            Logger.Instance.Info("[SDK.OBS][Service.WebSocket_OnMessageReceived]");
#endif
...
#if DEBUG
                    Logger.Instance.Info("[SDK.OBS][Service.WebSocket_OnMessageReceived] UpdateType: " + l_UpdateType);
#endif
                    switch (l_UpdateType)
                    {
...
#if DEBUG
                        default:
                            Logger.Instance.Error("[SDK.OBS][Service.WebSocket_OnMessageReceived] Unhandled:");
                            Logger.Instance.Error(p_Message);
                            break;
#endif
                    }
            ...
            {
                Logger.Instance.Error("[SDK.OBS][Service.WebSocket_OnMessageReceived] Error:");
        ...
        {
            Logger.Instance.Info("[SDK.OBS][Service.WebSocket_OnError]");
        }

...
#pragma warning restore CS0649
        ...
        public async Task TryRequestResources(string p_Category, string p_Token)
        {
            ...
            if (l_IsGlobal)
            {
                try
                {
                    Logger.Instance.Debug($"Requesting 7TV cosmetics");

                    using (HttpRequestMessage msg = new HttpRequestMessage(HttpMethod.Get, "https://api.7tv.app/v2/cosmetics?user_identifier=twitch_id"))
                    {
                        var l_Response = await m_HTTPClient.SendAsync(msg);
                        if (l_Response.IsSuccessStatusCode)
                        {
                            var l_Cosmetics = JsonConvert.DeserializeObject<Models.Twitch._7TVCosmetics>(await l_Response.Content.ReadAsStringAsync());
                            ...
                            Logger.Instance.Debug($"Success caching 7TV cosmetics ({m_Paints.Count} Paints).");
                        }
                        else
                            Logger.Instance.Error($"Unsuccessful status code when requesting 7TV cosmetics. {l_Response.ReasonPhrase}");
                    }
                }
                catch (Exception l_Exception)
                {
                    Logger.Instance.Error($"An error occurred while requesting 7TV cosmetics.");
                    Logger.Instance.Error(l_Exception);
                }
                ...
                try
                {
                    ...
                    using (HttpRequestMessage msg = new HttpRequestMessage(HttpMethod.Get, "https://mods-data.hardcpp.com/twitch_painted_names.json"))
                    {
                        var l_Response = await m_HTTPClient.SendAsync(msg);
                        if (l_Response.IsSuccessStatusCode)
                        {
                            var l_CustomPaintings   = JsonConvert.DeserializeObject<CustomPaint[]>(await l_Response.Content.ReadAsStringAsync(), new Config.JsonConverters.Color32Converter());
                            var l_Count             = 0;
                            ...
                            Logger.Instance.Debug($"Success caching custom cosmetics ({l_Count} Paints).");
                        }
                        else
                            Logger.Instance.Error($"Unsuccessful status code when requesting custom cosmetics. {l_Response.ReasonPhrase}");
                    }
                ...
                {
                    Logger.Instance.Error($"An error occurred while requesting custom cosmetics.");
                    Logger.Instance.Error(l_Exception);
                }
            }
        }

    ...
    public static class DictionaryUtils
    {
        ...
        public static void InvokeAll(this ConcurrentDictionary<ChatServiceBase, Action> dict)
        {
            foreach (var kvp in dict)
            {
                ...
                {
                    ChatPlexSDK.Logger.Error($"An exception occurred while invoking action no params.");
                    ChatPlexSDK.Logger.Error(ex);
                }
            }
        }
        ...
        public static void InvokeAll<A>(this ConcurrentDictionary<ChatServiceBase, Action<A>> dict, A a)
        {
            foreach (var kvp in dict)
            {
                ...
                {
                    ChatPlexSDK.Logger.Error($"An exception occurred while invoking action with param type {typeof(A).Name}");
                    ChatPlexSDK.Logger.Error(ex);
                }
            }
        }
        ...
        public static void InvokeAll<A, B>(this ConcurrentDictionary<ChatServiceBase, Action<A, B>> dict, A a, B b)
        {
            foreach (var kvp in dict)
            {
                ...
                {
                    ChatPlexSDK.Logger.Error($"An exception occurred while invoking action with param types {typeof(A).Name}, {typeof(B).Name}");
                    ChatPlexSDK.Logger.Error(ex);
                }
            }
        }
        ...
        public static void InvokeAll<A, B, C>(this ConcurrentDictionary<ChatServiceBase, Action<A, B, C>> dict, A a, B b, C c)
        {
            foreach (var kvp in dict)
            {
                ...
                {
                    ChatPlexSDK.Logger.Error($"An exception occurred while invoking action with param types {typeof(A).Name}, {typeof(B).Name}, {typeof(C).Name}");
                    ChatPlexSDK.Logger.Error(ex);
                }
            }
        }
        public static void InvokeAll<A, B, C, D>(this ConcurrentDictionary<ChatServiceBase, Action<A, B, C, D>> dict, A a, B b, C c, D d)
        {
            foreach (var kvp in dict)
            {
                ...
                {
                    ChatPlexSDK.Logger.Error($"An exception occurred while invoking action with param types {typeof(A).Name}, {typeof(B).Name}, {typeof(C).Name}, {typeof(D).Name}");
                    ChatPlexSDK.Logger.Error(ex);
                }
            }
        }

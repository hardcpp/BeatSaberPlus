...
namespace CP_SDK_LiteNetLib.Utils
{
    public class NetPacketProcessor
    {
        ...
        public delegate void ActionRef<T, TUserData>(ref T item, TUserData userData);
        public void SubscribeNetSerializableRef<T, TUserData>(
            ActionRef<T, TUserData> onReceive) where T : INetSerializable, new()
        {
            var reference = new T();
            _callbacks[GetHash<T>()] = (reader, userData) =>
            {
                reference.Deserialize(reader);
                onReceive(ref reference, (TUserData)userData);
            };
        }

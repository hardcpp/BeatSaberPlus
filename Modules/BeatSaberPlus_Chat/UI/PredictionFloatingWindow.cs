...
namespace ChatPlexMod_Chat.UI
{
    ...
    /// </summary>
    internal class PredictionFloatingWindow : BeatSaberPlus.SDK.UI.ResourceViewController<PredictionFloatingWindow>
    {
...
#pragma warning restore CS0649
        ...
        protected override sealed void OnViewCreation()
        {
            ...
            GetComponentInChildren<ImageView>().material    = BeatSaberPlus.SDK.Unity.MaterialU.UINoGlowMaterial;
        ...
        private void SetupTimeFrame()
        {
            ...
            l_Background.sprite     = CP_SDK.Unity.SpriteU.CreateFromTexture(Texture2D.whiteTexture);
            l_Background.type       = Image.Type.Filled;
            l_Background.fillMethod = Image.FillMethod.Horizontal;
            l_Background.fillAmount = 1f;
            l_Background.color      = TIME_PROGRESSBAR_BACKGROUND;
            l_Background.material   = BeatSaberPlus.SDK.Unity.MaterialU.UINoGlowMaterial;

            var l_Filler = m_TimeFrame.transform.GetChild(0).gameObject.AddComponent<Image>();
            l_Filler.sprite     = CP_SDK.Unity.SpriteU.CreateFromTexture(Texture2D.whiteTexture);
            l_Filler.type       = Image.Type.Filled;
            l_Filler.fillMethod = Image.FillMethod.Horizontal;
            l_Filler.fillAmount = 1f;
            l_Filler.color      = TIME_PROGRESSBAR_FILLER;
            l_Filler.material   = BeatSaberPlus.SDK.Unity.MaterialU.UINoGlowMaterial;
            ...
            if (p_Prediction != null)
            {
                ...
                else
                {
                    ...
                    CP_SDK.Unity.MTMainThreadInvoker.Enqueue(() =>
                    {
                        ...
                        m_PickButtons.SetActive(p_Prediction.status == CP_SDK.Chat.Models.Twitch.Helix_Prediction.Status.LOCKED
                                                || p_Prediction.status == CP_SDK.Chat.Models.Twitch.Helix_Prediction.Status.RESOLVED);

                        m_CancelButton.interactable = p_Prediction.status != CP_SDK.Chat.Models.Twitch.Helix_Prediction.Status.RESOLVED;
